...
async fn function_handler(alb_url: &str, event: LambdaEvent<HashMap<String, String>>) -> Result<(), Error> {
    ...
    if passed {
        if let Err(_) = run_test(alb_url, "two".to_string()).await {
            info!("Test on Route two failed, rolling back");
            passed = false
        }
    }

    if passed {
        if let Err(_) = run_test(alb_url, "three".to_string()).await {
            info!("Test on Route three failed, rolling back");
            passed = false
        }
    }
